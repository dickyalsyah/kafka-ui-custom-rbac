...
public class BasicAuthSecurityConfig extends AbstractAuthSecurityConfig {
  ...
  @Bean
  @ConditionalOnProperty(prefix = "auth.basic", name = "users[0].username")
  public MapReactiveUserDetailsService userDetailsService(BasicAuthProperties properties) {
    log.info("Configuring custom users for LOGIN_FORM authentication.");
    return new MapReactiveUserDetailsService(
        properties.getUsers().stream()
            .map(u -> new BasicUser(u.getUsername(), "{noop}" + u.getPassword(), u.getRoles()))
